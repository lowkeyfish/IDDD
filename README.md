...
领域驱动设计（Domain-Driven Design，简称 DDD）是一种软件开发方法论，由 Eric Evans 在 2003 年提出。他在 2003 年出版了一本名为《领域驱动设计：软件核心复杂性应对之道》（Domain-Driven Design: Tackling Complexity in the Heart of Software）的著作，详细介绍了这一方法论。我们频繁地听到 DDD 的名字，也经常讨论它使用它，但它对于大部分人来说却慢慢变得和面向对象 编程（Object Oriented Programming，简称 OOP）一样，看似已经融入了我们的编程生活，但真正理解它并用好它却并不容易。

了解过甚至尝试过 DDD 的开发者大概率都是在编程中遇到了问题，期望通过一种方式帮自己解决问题。我们希望 DDD 就是帮我们解决问题的那种方式。涉及 DDD 的书籍和文章不计其数，这正说明了 DDD 的价值，但是大部分开发者未能从中找到落地实践 DDD 的方法，特别是浅显易懂的方法。从本人实践的经历看，经典书籍无论我们何时翻阅，总能在其中找到新的启示和认识，但对于新手来说有些过于抽象不够直白，示例项目也往往和我们的实际的需求和经历相去甚远。最终无论我们看了多少书，实践时总是束手无策。甚至于许多开发者简单的认为是自己项目业务太简单了，但就是这些简单的项目把我们搞得焦头烂额。
...
代码即文档，这是很多开发者经常挂在嘴边的一句话，虽然经常用来作为不写注释的理由。但这是有前提的，能够良好体现业务需求的代码才是文档。只能由机器执行得到结果的代码还远远称不上文档，只能从这些机器才能理解的代码中了解需求的困境应该很多人都体验过，但是针对这个问题大家既是受害者也是始作俑者。好的代码不仅是需求的一种实现方式，同时也能体现出需求的意图，反之不好的代码总是让人产生有一种它为什么这么写的疑惑。
...
理论结合实践才能获得更深的理解和更全面的学习，整篇文章的讨论我们都基于一个汽车经销商网上店铺的项目进行讨论。汽车经销商网上店铺的示例项目主要分为两个子项目项目：产品库项目和网上店铺项目。产品库项目主要用于管理汽车品牌、车系和车型数据，之所以独立出一个项目主要是为了演示 DDD 中如何依赖其他系统提供的功能。网上店铺项目主要包含经销商管理、活动管理、活动礼品管理、顾问管理和经销商在售车系车型的管理。示例项目的完整源代码可以通过 https://github.com/lowkeyfish/IDDD 获取。
...
示例项目中经销商可以发布活动，按以往的编程方式，我们先在数据库建一个活动表（activity），表中的主键作为活动的 ID。代码中会有一个和数据表字段对应的类，这个数据类对外方法只有 getter 和 setter，调用 setter 方法就可以直接修改数据，数据是否可以修改，修改成什么值都是有事务脚本也就是我们经常说的 service 控制。在 DDD 中，我们会将活动相关的业务概念建模为实体（Activity），实体的属性用于存储活动相关的业务数据，但并不一定要按照数据库表的结构去设计实体的属性。且在实体中你看不到之前熟悉的公共的 getter 和 setter 方法，因为实体不仅是数据同时也是业务行为的封装。借助实体，调用者的关注点从数据操作转变为了业务行为的执行。

对于不需要唯一标识且具有不变性的对象在 DDD 中被称为值对象，值对象的等价性是基于它们的属性值，而不是某个唯一标识。值对象一般被用作一组相关属性的集合，实体 `Activity` 中唯一标识 `ActivityId` 和用作时间范围的 `TimeRange` 都属于值对象。

对于活动场景，除了 `Activity` 实体还要使用表示活动礼品的实体 `ActivityGift`。对于活动业务场景中涉及的这些实体和值对象，在整个生命周期中作为一个整体维护它们的一致性和完整性是很有挑战的。为此 DDD 提出了聚合的领域概念。聚合有一个聚合根，聚合根是聚合内的一个特定实体，对于活动聚合来说，我们选用 `Activity` 实体作为聚合根。由 `Activity` 作为聚合根管理活动相关的业务数据，并对外提供业务方法管理内部实体和值对象的一致性和完整性。
...
聚合根和实体需要为不同场景提供不同的有参构造函数，分别用于调用方首次创建聚合根和通过仓库查询创建聚合根。首次创建聚合根时使用的构造函数仅需提供核心参数即可，仓库重建聚合根需要提供全部的属性值，这些值使用上次聚合根已保存数据。
...
* 参数不能直接提供聚合根内部实体的引用。例如创建聚合根时礼品参数类型为 `Map<GiftId, Integer>` 而非 `List<ActivityGift>`，主要是为了防止外部绕过聚合根直接修改实体的数据状态。
* 在构造函数中需要对接收的参数进行数据一致性和完整性校验。如果不满足需要直接抛出异常。而用于仓库重建聚合根的构造函数不需要再对参数进行校验。

#### 聚合根是聚合的唯一入口

通过聚合和聚合根，我们可以将相关的实体和值对象组织在一起，定义严格的边界，封装内部的业务规则，确保业务的一致性和完整性，聚合根是对外界可见的唯一入口。聚合根不能够对外提供聚合根内部实体的引用。因为一旦外部可以获取到内部的实体引用就可以绕过聚合根修改实体的状态，这些操作会导致聚合根整体数据的一致性和完整性问题。
...
命令查询职责分离（Command Query Responsibility Segregation，简称 CQRS）模式将应用的读操作和写操作分离开来，使得我们可以独立地扩展和优化它们。将聚合根作为命令模型实现可以大大降低聚合根的复杂性，按照命令模型实现的聚合根中执行业务操作的方法不需要返回值，且不提供查询方法，例如报名和活动有关联关系，但查询活动报名用户不会在活动聚合根中提供。

#### 聚合根数据验证的范畴

活动聚合根使用经销商唯一标识 `DealerId` 标识其关联的经销商。业务中我们必须保证使用的唯一标识是确实有对应经销商的，不能是一个无效的 ID。这个规则就涉及到通过 ID 获取到 `Dealer` 聚合根并判断其是否为 `null`。创建活动聚合根时接收参数 `dealerId`，但只需要验证 `dealerId` 是否为 null 即可。`dealerId` 是否存在经销商的判断不是活动聚合根的责任，但是这个验证确实属于业务规则，业务规则应该不包含在领域层，我们需要为此提供领域服务，然后由应用服务使用领域服务完成这些业务规则的验证。

总结来说聚合根（包括用于创建聚合根的工厂）仅需对聚合自身数据进行一致性和完整性验证，自身数据关联对象的验证虽然也属于业务规则但并不属于聚合根的职责。为什么这样做是更好的呢。假设我们想让 `Activity` 聚合根创建时验证其关联的 `dealerId` 必须有对应的 `Dealer`。我们可以通过以下几点分析下带来哪些问题：

* 如果接收 `DealerId`，那获取到 `Dealer` 并验证就需要 `Activity` 具有查询 `Dealer` 的能力，就需要让 `Activity` 增加对 `DealerRepository` 的依赖。如果你需要同时创建多个 `Activity` 的实例，就会存在多次重复获取 `Dealer`，这是不必要的重复操作。
* 你可能想到可以在应用服务层直接获取到 `Dealer` 然后自己不判断，传给 `Activity` 判断。虽然这样可以避免重复获取 `Dealer` 的问题，但是 `Activity` 还是增加了对 `Dealer` 的依赖，这实际上是没必要的。
* 即使你不考虑额外依赖 `Dealer` 的问题。创建 `Activity` 时传的是 `Dealer`，构造函数中就必须验证 `Dealer` 是否为 null，否则就可能在获取 `dealerId` 时遇到空引用问题。但是如果应用服务层获取到 `Dealer` 后也需要做其他的规则判断，例如，经销商是否有某类操作权限，那么应用服务层也需要对 `Dealer` 做非 null 判断。这样就出现了多处对同一业务规则做判断的重复性操作。

#### 聚合根的依赖

聚合根一般应该视为是一个用时创建用完回收的短生命周期对象。聚合根不能像 Service 一样初始化时就依赖其他领域服务或资源库。

聚合根在初始化后就可以看作是一个内存对象了，聚合根提供的方法有时候还需要基于其他领域服务保证聚合数据的一致性和完整性，聚合根依赖这些领域服务的方式是将领域服务作为方法参数传入。例如活动支持重命名，`Activity` 通过方法 `rename(name, activityNameUniquenessCheckService)` 完成活动的重命名。业务要求活动不能重名，`Activity` 自身并不支持这个业务规则判断，这就需要依赖领域服务 `ActivityNameUniquenessCheckService`，聚合根 `Activity` 的构造函数并不会依赖领域服务，而是在需要使用的方法上通过参数来依赖领域服务。

聚合根不对外提供数据查询，也不关注自身的获取和持久化，因此聚合根不需要也不能依赖仓库。
...
在可能的情况下尽量将聚合根设计的足够小。通俗来说就是把聚合根设计能够满足其对应的业务场景即可，例如实际业务中活动有参与人，我们将活动和参与人创建两个聚合根：聚合根 `Activity` 和聚合根 `ActivityRegistration`，而不是让 `ActivityRegistration` 作为 `Activity` 的一部分。如果只有 `Activity` 聚合根，内部使用 `List<ActivityRegistration>` 表示活动的参与人，每个 `ActivityRegistration` 的状态变更都必须通过 `Activity` 操作，这对并发控制和性能优化非常不利。
...
在 DDD 中，我们通过聚合根、实体和值对象封装业务逻辑。这些对象不仅包含数据，还包含操作数据的行为。我们应避免通过领域对象查询其数据状态，然后在外部进行业务逻辑判断后再修改它的数据。应该直接告诉领域对象要做什么，让它自己完成操作。例如，活动重命名时要求活动当前状态是未开启报名的状态，只有未开始报名才允许修改活动名称。旧的编程方式调用方会主动询问 `Activity` 的当前状态，如果状态满足业务规则，调用方会再调用其更新名称的方法。在 DDD 中，调用方只需要直接调用 `Activity.rename()` 方法接口，聚合根内部自己保证了只有状态满足时才会更新活动名称，否则将抛出异常中断当前操作。
...
* 在单个事务中只操作一个聚合根的原则，有助于我们避免处理修改多个聚合根以保证数据一致性所带来的复杂性。当我们需要在单个事务中修改多个聚合根时，通常需要使用更复杂的技术，如分布式事务，以确保数据一致性。这不仅增加了系统的复杂性，还可能带来性能问题。
* 在单个事务中只操作一个聚合根，使得服务拆分变得更简单，更安全。
* 配合领域事件的使用，可以使聚合根或服务之间的耦合度大大降低，使我们的代码更加灵活，更易于维护和扩展。
...
例如活动聚合根的创建，需要获取用于创建活动的唯一标识以及对活动名称做唯一校验。构造函数无法承担这些操作，它们作为领域层职责需在领域层完成。因此针对活动聚合根的创建使用单独的工厂类比较合适。

一般还是建议优先使用聚合根的构造函数，只有构造函数无法满足时再尝试使用独立的工厂类。

#### 聚合根用于自身元素添加而提供的工厂方法

在聚合根上添加工厂方法用于为自身添加元素可以把聚合根的内部实现细节隐藏起来，同时聚合根也可以确保添加元素时其自身数据的完整性。例如，活动聚合跟提供用于添加礼品的工厂方法用于为自己添加更多的礼品。

#### 领域对象为其他对象创建而提供的工厂方法

一个对象与另一个对象密切相关，但其并不拥有所生成的对象。当一个对象的创建主要使用另一个对象的数据或者规则时，可以在后者的对象上创建一个工厂方法，这样就不必将后者的信息暴露出来。例如活动报名场景中就是通过活动聚合根通过方法 `register()` 创建报名聚合根 `ActivityRegistration`。
    
使用这种类型的工厂方法时我们需要避免一些误区。例如，大部分聚合根和 `Dealer` 有关系，即大部分聚合根的创建都需要 `DealerId`，那么应该在 `Dealer` 中为这些和它有关的聚合根创建工厂方法吗？

聚合根创建需要 `DealerId`，你需要对 `DealerId` 校验其是否存在 `Dealer`，甚至还要判断 `Dealer` 的一些状态或权限。基于这些考虑，你可能会觉得在 `Dealer` 中为其它聚合根创建工厂方法好像挺好的，甚至于使用起来还挺面向对象的。那我们应该这么做吗，结论当让是没有必要这么做，甚至于要避免这样做。
...
总结来说，如果两个聚合根处于同一个业务场景中，并且他们之间有强业务规则关联，那么在一个聚合根中为另一个聚合根提供工厂方法是很有意义的。这样可以保证业务规则的实施，并且使得领域模型更加符合业务逻辑。反之，就没有必要在一个聚合根中为另一个聚合根提供工厂方法，这样可以避免不必还要的耦合，也使得领域模型更加清晰，也更容易维护和扩展。


### 资源库（Repository）

资源库用于存储、检索和删除聚合根。每个聚合根都有其对应的资源库，且只有聚合根才能有资源库，每个资源库也仅用于一个聚合根。资源库的命名一般以聚合根名称加后缀 `Repository` 组成。例如聚合根 `Activity` 其资源库为 `ActivityRepository`。

聚合根是聚合对外的唯一入口，因此资源库操作的对象必须是聚合根，即资源库保存的对象是聚合根，查询出的对象还是聚合根，这样可以确保聚合的一致性。

资源库一般会针对其聚合根提供必须的几个方法，例如 `ActivityReponsitory` 提供了 `findById(ActivityId)`、`save(Activity)` 和 `remove(Activity)`。
...
* 无法将逻辑自然的分配给聚合根、实体或值对象。
	...
	例如活动名称需要满足唯一性检查，这个操作无法放到活动聚合根中，只能作为领域服务实现。
...
聚合根创建成功的场景，一般可以在聚合根的构造函数中发布领域事件。聚合根状态变更的场景一般由对应的聚合根方法发布领域事件。例如，聚合根 `Activity` 用于创建的构造函数会发布领域事件 `ActivityCreated`，活动更新方法 `update()` 会发布领域事件 `ActivityUpdated`。
...
* 事件名称
	...
	事件名称应该清晰地表达出业务发生了什么事情。例如 `ActivityCreated`、`ActivityUpdated`。
    
* 事件源的标识

	事件源就是触发事件的聚合根或实体，它的标识符应该包含在领域事件中，以便知道是哪个对象发布了事件。例如，活动聚合根 `Activity` 发布了一个 `ActivityCreated` 事件，事件应该包含 `ActivityId`。
...
* 协调和驱动领域对象进行工作
	...
	应用服务协调多个领域对象，以实现一个完整的业务流程。应用服务不应该包含业务规则或复杂的业务逻辑，自身应该作为一个协调者，例如，`DealerApplicationService` 协调了`DealerFactory` 和 `DealerRepository` 来完成创建新的 `Dealer` 聚合根并进行持久化的操作。而关于如何创建 `Dealer` 和如何进行持久化，以及涉及的所有业务规则，都是由 `DealerFactory` 和 `DealerRepository` 来负责的。
...
> ![Domain-Driven Design](https://note.yujunyang.com/static/2023/7/5d2a33ad09423b54f0275c2e21276655.png)

> 《实现领域驱动设计》（Implementing Domain-Driven Design）Vaughn Vernon  
> ![Implementing Domain-Driven Design](https://note.yujunyang.com/static/2023/7/44cef68fe8468f5fe45277e901357bb0.png)

> 《Patterns, Principles, and Practices of Domain-Driven Design》Scott Millett / Nick Tune  
> ![Patterns, Principles, and  Practices of Domain-Driven Design](https://note.yujunyang.com/static/2023/7/ef36551aba472481e2ee42e59209f0e0.png)
